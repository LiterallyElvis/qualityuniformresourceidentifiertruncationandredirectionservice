...
func (c *Chain) Generate(n int, start string) string {
	p := make(Prefix, c.prefixLen)
	var words []string
	...
	for i := 0; i < n; i++ {
		...
		nextChoiceIndex := rand.Intn(len(choices))
		next := choices[nextChoiceIndex]
...
func buildMarkov() *Chain {
	rand.Seed(time.Now().UnixNano()) // Seed the random number generator

	c := NewChain(2) // Initialize a new Chain.
	source, err := os.Open("markov.txt")
	if err != nil {
		log.Fatal(err)
	}
	c.Build(source) // Build chains from standard input.
	return c
}
